...
def sort_contour(contours, from_idx, to_idx, sortOrder):
    if sortOrder == "top-to-bottom":
        contours[from_idx:to_idx] = sorted(contours[from_idx:to_idx], key=lambda c: cv2.boundingRect(np.array(c))[1])
    elif sortOrder == "left-to-right":
        contours[from_idx:to_idx] = sorted(contours[from_idx:to_idx], key=lambda c: cv2.boundingRect(np.array(c))[0])
