...
# Extracting answers using the provided block_info list
all_detected_answers = {}
for block in block_info:
    detected_answers = extract_answers_from_block("/Users/henrychun/Downloads/Math-Thinking-NSW.jpeg", block)
    adjusted_answers = {int(k) + int(block['StartingQuestion']) - 1: v for k, v in detected_answers.items()}
    all_detected_answers.update(adjusted_answers)

# Sorting the detected answers based on question number
sorted_answers = dict(sorted(all_detected_answers.items()))
# Printing the detected answers
for question, answer in sorted_answers.items():
